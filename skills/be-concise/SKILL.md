...
Answer first, use plain language, and stop when the answer is complete. Aim for about a third of the usual length by selecting what matters, not by breaking grammar or dropping an important caveat.
...
- `/be-concise` activates it explicitly. An always-on installation requires the separate work-discipline layer or equivalent harness instructions.
...
Installing a model-invoked skill makes it discoverable, not permanently active. Explicit `/be-concise` applies to the current request and, where the harness preserves invoked-skill state, the current session. The work-discipline layer supplies the portable always-on form. A depth request or "go long" lifts the constraint for that reply.
...
These are common reasons concise answers become longer or less clear than necessary.
...
- **The skill is installed but does not trigger:** invoke `/be-concise` explicitly or install the work-discipline layer for always-on behavior; installation alone is not activation.
